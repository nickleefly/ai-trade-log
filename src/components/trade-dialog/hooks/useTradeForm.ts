...
export const useTradeForm = ({ editMode = false, existingTrade, day, onRequestClose }: UseTradeFormProps) => {
    ...
    // Define default values as constants to ensure consistent typing
    const createDefaultValues: z.infer<typeof newTradeFormSchema> = {
        positionType: "buy",
        openDate: "",
        openTime: "12:30",
        closeDate: "",
    ...
    const getDefaultValues = (): z.infer<typeof newTradeFormSchema> => {
    ...
    const form = useForm<z.infer<typeof newTradeFormSchema>>({
        resolver: zodResolver(newTradeFormSchema) as any,
